...
def escape_md(text: str) -> str:
    """Escape karakter khusus Telegram Markdown (v1) agar tidak crash."""
    # Karakter yang perlu di-escape di Markdown v1: _ * ` [
    for ch in ("_", "*", "`", "["):
...
async def _ask_provider(message, context, handle: str):
    ...
    await message.reply_text(
        f"🤖 *Pilih AI Provider:*\n\n"
        f"🌐 URL: `{escape_md(context.user_data.get('url',''))}`\n"
        f"🐦 Twitter: `{escape_md(handle_info)}`\n\n"
...
async def _run_generation(
    ...
    try:
        ...
        await message.reply_text(
            f"✅ *Thread selesai!*\n\n"
            f"📌 *{escape_md(result['project_name'])}*\n"
            f"🧵 {result['tweet_count']} tweets · {lang_label}\n"
            f"{tw_status}"
            f"{media_str}"
            f"{used_emoji} Ditulis oleh *{escape_md(used_name)}* (`{escape_md(used_model)}`)\n\n"
            f"⬇️ Thread lengkap:",
            parse_mode=ParseMode.MARKDOWN,
        )
...
def _safe_markdown(text: str) -> str:
    ...
    # Escape underscore karena sering muncul di nama project/token
